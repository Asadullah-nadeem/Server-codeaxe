...
import { Row, Col, Card, Button, Modal, Badge, ProgressBar, Form } from 'react-bootstrap';
...
const MediaGallery = ({ show, onHide, onSelect }) => {
    ...
    const handleUpload = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api'}/admin/dms/media`, {
                method: 'POST',
                            ...
                            <Col key={item.id} xs={4} md={3} lg={2} className="mb-3">
                                ...
                                >
                                    <div style={{ aspectRatio: '1/1', background: '#fff', overflow: 'hidden' }}>
                                        <img src={item.path} alt={item.file_name} className="w-100 h-100 object-fit-cover" />
                                    </div>
